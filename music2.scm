...
(define (rpitch4)
  (set! rpitch4-count (+ rpitch4-count))
  (+ 60 (list-ref rpitch4-list (modulo (- rpitch4-count 1) (length rpitch4-list)))))
(define rpitch4-count 0)
(define (generate-pitches h)
  ;(define vlist (list 0 1 -1 0))
  (define vlist (generate-list (lambda () (list (- (random-integer 11) 5)))
                               4))
  (define (addlist l n)
    (map (lambda(v) (+ v n)) l))
  (define (vlist-loop vlist l)
    (if (null? vlist)
      '()
      (append (addlist l (car vlist))
              (vlist-loop (cdr vlist) l))))
  (if (zero? h)
    vlist
    (vlist-loop vlist (generate-pitches (- h 1)))))
(define rpitch4-list (generate-pitches 3))
...
(define (rlength2)
  ;(define rythm '(4 4   8/3 8
  ;                4 8 8 8/3 8
  ;                8 8 4 8/3 8))
  (define rythm '(4 4 4 4))
